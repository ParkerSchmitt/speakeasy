...
class TopicRepository {
  ...
  async receiveTopicPercentage (userId: number, topic: string): Promise<{ percentageLearned: number } | null> {
    ...
    const query = `SELECT (COUNT(${this.cardAccountLinkageTableName}.id) * 1.0 / COUNT(${this.cardTableName}.id)) AS "percentageLearned" FROM ${this.topicTableName} topicTable INNER JOIN ${this.cardTableName} cardsTable ON topicTable.id = cardsTable."topicId" LEFT JOIN ${this.cardAccountLinkageTableName} cardAccountLinkageTable on cardsTable.id = cardAccountLinkageTable.card_id  AND cardAccountLinkageTable.account_id = ${userId} WHERE topicTable.name = '${topic}' GROUP BY topicTable.id`
